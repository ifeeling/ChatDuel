...
import { createCopilotAdapter } from '../adapters/copilot/adapter'
import { loadSelectorOverrides } from './selector-overrides'

const PLATFORM: AIPlatform = 'copilot'

async function boot() {
  const adapter = createCopilotAdapter(await loadSelectorOverrides(PLATFORM))
  ...
  function getProbeState(): ConversationState {
    ...
    return { status: 'queued', errorMessage: 'Copilot 页面已注入，但尚未识别到输入框' }
  }
