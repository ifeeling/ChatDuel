...
- **待发送问题管理**：等待 AI 回答时，可在问题真正发送前编辑、删除并调整顺序。待发送问题可以带一个附件，并在发送前查看、移除或替换；列表内附件合计上限为 50MB。没有待发送问题时，该区域会自动隐藏。
...
- **附件**：图片、TXT、CSV 会内联进文字发送；Markdown、HTML、PDF、Word、Excel 会作为真实文件上传。其它/未识别的文件类型也会自动尝试上传（先直塞输入框，不行再试粘贴），目标平台确实不支持才会退回人工上传；可执行/脚本文件（`.exe`/`.sh` 等）一律直接拒绝。
...
- **历史保存失败恢复**：当回答已捕获但本地历史保存失败时，待发送列表会暂停，并提供"重试保存历史"或"不保存，继续"两个操作，方便你重试保存或直接继续，避免丢失正在处理的回答。
- **官网会话**：保存官方网页的具体会话链接和当时显示的 AI 状态，用来回到以前的官方对话继续聊；它不保存 AI 回复正文。
- **升级提醒**：ChatDuel 升级后，下次打开扩展页面会一次性提醒你刷新已经打开的 AI 官网标签页；ChatDuel 不会自动刷新或关闭这些标签页。
