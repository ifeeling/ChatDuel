...
describe('deepseek adapter', () => {
  ...
  it('records Enter retries and acceptance without creating a text-only vision event', async () => {
    document.body.innerHTML = '<textarea placeholder="给 DeepSeek 发送消息"></textarea>'
    const textarea = document.querySelector<HTMLTextAreaElement>('textarea')!
    let keydowns = 0
    textarea.addEventListener('keydown', () => {
      keydowns += 1
      if (keydowns === 2) textarea.value = ''
    })
    const trace = diagnostics()

    await createDeepSeekAdapter().sendMessage('PRIVATE_PROMPT', undefined, trace.value)
