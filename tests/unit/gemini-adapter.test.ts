...
function diagnostics() {
  const emit = vi.fn()
  return { emit, value: { reporter: { emit }, selectorConfigVersion: '2026.06' } satisfies AdapterDiagnostics }
}
...
describe('gemini adapter', () => {
  ...
  it('retries Enter when the first submit leaves the message in the editor', async () => {
    ...
    await createGeminiAdapter().sendMessage('这是什么?', undefined, trace.value)

    expect(keydowns).toBe(2)
    expect(trace.emit).toHaveBeenCalledWith(expect.objectContaining({ operation: 'send-click', retryNumber: 1 }))
    expect(trace.emit).toHaveBeenCalledWith(expect.objectContaining({ operation: 'send-click', retryNumber: 2 }))
    expect(trace.emit).toHaveBeenLastCalledWith(expect.objectContaining({
      operation: 'send-ack', stage: 'accepted', retryCount: 2,
    }))
